...
public class CategoryController {
    ...
    public HttpEntity<?> pageable(@RequestParam(name = "page",defaultValue = AppConstants.DEFAULT_PAGE_NUMBER)Integer page,
        ...
        return ResponseEntity.ok(categoryService.pageable(page,size,search));
    }

    @GetMapping("/{id}")
    public ApiResponse changeActive(@PathVariable Integer id,@RequestParam boolean active){
        return categoryService.changeActive(id,active);
    }
    ...
    @DeleteMapping("/{id}")
    public ApiResponse removeCategory(@PathVariable Integer id){
